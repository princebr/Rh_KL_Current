	...
	if (rl.dim == ON)
	{
		...
		if (bcm2835_st_read() >= rl.timerExp)
		{
			...
			if ((rl.inc_dec == INC) & (rl.pwmRaw_R >= rl.pwmTarget_R))
			{
				...
				// hit final target explicitly
				rl.pwmRaw_R = rl.pwmTarget_R;
				rl.pwmRaw_G = rl.pwmTarget_G;
				rl.pwmRaw_B = rl.pwmTarget_B;
			}
...
void svc_LAY_usw(void)
{
	...
	// Detect level on LAY uSW
		...
		{
			// Initiate RGB Dimming Features
			backshellLight = dimRGBStart(backshellLight, ttlRGB);//neuRGB);
			stowageLight = dimRGBStart(stowageLight, ttlRGB);//neuRGB);
			footwellLight = dimRGBStart(footwellLight, ttlRGB);//neuRGB);
			underIFELight = dimRGBStart(underIFELight, ttlRGB);//neuRGB);
